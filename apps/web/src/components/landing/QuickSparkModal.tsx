...
export const QuickSparkModal = ({ isOpen, onClose }: QuickSparkModalProps) => {
  const [step, setStep] = useState(0); // 0-2: Questions, 3: Result/Form
  const [score, setScore] = useState(0);
  const [selectedOption, setSelectedOption] = useState<number | null>(null);

  const handleAnswer = (index: number) => {
    setSelectedOption(index);
    if (index === QUIZ_QUESTIONS[step].correct) setScore(score + 1);
            ...
            <GlassCard className="rounded-t-[32px] md:rounded-[32px] border-white/60 min-h-[500px] flex flex-col">
              ...
              <div className="flex justify-between items-center mb-8">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[#162B6E] font-bold text-sm tracking-tight">Quick Spark</span>
                </div>
                ...
                {step < QUIZ_QUESTIONS.length ? (
                  ...
                  >
                    <div className="space-y-2 text-center md:text-left">
                      <p className="text-[#24A5EE] font-bold text-xs uppercase tracking-[0.2em]">Challenge {step + 1} of 3</p>
                      <h3 className="text-2xl font-bold text-[#162B6E]">{QUIZ_QUESTIONS[step].question}</h3>
                    </div>
                      ...
                      {QUIZ_QUESTIONS[step].options.map((option, i) => (
                        <button
                          ...
                          className={`w-full p-5 text-left rounded-2xl border-2 transition-all font-medium flex justify-between items-center
                            ${selectedOption === i 
                              ? (i === QUIZ_QUESTIONS[step].correct ? 'border-green-500 bg-green-50 text-green-700' : 'border-red-500 bg-red-50 text-red-700')
                              : 'border-white/80 bg-white/60 hover:border-[#24A5EE] text-[#162B6E] shadow-sm'}
                          `}
                        >
                          {option}
                          {selectedOption === i && i === QUIZ_QUESTIONS[step].correct && <CheckCircle2 size={20} />}
                        </button>
                      ))}
                    </div>
                  </motion.div>
                  ...
                  >
                    ...
                    <h3 className="text-3xl font-bold text-[#162B6E]">Heritage Score: {Math.round((score/3)*100)}%</h3>
                    <p className="text-slate-500">You have a natural ear for Yorùbá! Save your progress and start your 7-day free trial to unlock your full Mastery Path.</p>
                    ...
                    <form className="space-y-3 pt-4" onSubmit={(e) => e.preventDefault()}>
                      ...
                      />
                      <button className="w-full py-5 bg-[#24A5EE] text-white rounded-2xl font-bold shadow-lg shadow-blue-400/30 flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
